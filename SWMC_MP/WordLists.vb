...
#Region "Neighborhood"
    ...
    Public Class OrthographicComparisonCorpus
        ...
        Public Sub ExportToTxtFile(ByVal saveDirectory As String, Optional ByRef saveFileName As String = "OLDComparisonCorpus",
    ...
    Public Class PLDComparisonCorpus
        ...
        Public Sub ExportToTxtFile(ByVal saveDirectory As String, Optional ByRef saveFileName As String = "PLDComparisonCorpus",
...
#Region "InputOutput"
    ...
    ''' <param name="DataOutputFilePathUsed">Upon return of the function, this variable holds the actual file path used."></param>
    ''' <returns>Returns True if the save procedure completed, and False is saving failed.</returns>
    Public Function SaveToTxtFile(ByVal saveDirectory As String,
...
0:
    ...
    Public Shared Function LoadWordGroupPhoneticTxtFile(ByVal filePath As String,
...
#Region "SyllableClasses"
    ...
    Public Class ListOfSyllables
        ...
        '    'This function was used earlier to create a deep copy, but as it is now concidered DANGEROUS to use binary serialization
        '    ' it's now been changed to using json serialization.
        ...
        Public Function CreateCopy() As ListOfSyllables
            ...
            'Serializing to memorystream
            Dim jsonSer = New System.Runtime.Serialization.Json.DataContractJsonSerializer(GetType(ListOfSyllables))
            Dim serializedMe As New MemoryStream
            jsonSer.WriteObject(serializedMe, Me)

            'Deserializing Me to a new object
            serializedMe.Position = 0
            Dim newListOfSyllables As ListOfSyllables = jsonSer.ReadObject(serializedMe)
            serializedMe.Close()
    ...
    Public Class Syllable
        ...
        Public Function CreateCopy() As Syllable
            ...
            'Serializing to memorystream
            Dim jsonSer = New System.Runtime.Serialization.Json.DataContractJsonSerializer(GetType(Syllable))
            Dim serializedMe As New MemoryStream
            jsonSer.WriteObject(serializedMe, Me)

            'Deserializing Me to a new object
            serializedMe.Position = 0 ' Is this line necessary?
            Dim SyllableCopy As Syllable = jsonSer.ReadObject(serializedMe)
            serializedMe.Close() ' Is this line necessary?
...
#Region "InputOutput"
    ...
    'Public Function CreateCopy_OLD() As Word
    ...
    Public Function CreateCopy() As Word
        ...
        'Serializing to memorystream
        Dim jsonSer = New System.Runtime.Serialization.Json.DataContractJsonSerializer(GetType(Word))
        Dim serializedMe As New MemoryStream
        jsonSer.WriteObject(serializedMe, Me)

        'Deserializing Me to a new object
        serializedMe.Position = 0
        Dim newWord As Word = jsonSer.ReadObject(serializedMe)
        serializedMe.Close()
...
#End Region
    ...
    ''' <summary>
    ''' Converts the current Word to a TextOnlyWord
    ''' </summary>
    ''' <returns></returns>
    Public Function ConvertToTextOnlyWord() As TextOnlyWord

        'Getting all properties of a TextOnlyWord
        Dim PropNameList As New List(Of String)
        Dim MyProperyInfo() As System.Reflection.PropertyInfo = GetType(TextOnlyWord).GetProperties
        For Each pi In MyProperyInfo
            PropNameList.Add(pi.Name)
        Next
        ...
        Dim CurrentPhoneCount As Integer = CountPhonemes(False)
    ...
    ''' <summary>
    ''' Generates a string representing a text value for the indicated Word Property.
    ''' </summary>
    ''' <param name="AllowIrrelevantValues">If set to false, fields that contain irrelevant data, (such as P2G data when no phonetic transcription exist) are exported as empty fields. If left to false, a default or irrelevant value is exported for some data types.</param>
    '''<param name="GenerateLackingData">If set to true, some data is automatically generated if lacking.</param>
    ''' <returns></returns>
    Public Function GetTextOnlyWordValue(ByVal PropertyName As String,
                                        Optional ByVal CurrentPhoneCount As Integer? = Nothing,
                                     Optional ByVal RoundingDecimals As Integer = 4,
                                     Optional ByVal AllowIrrelevantValues As Boolean = True,
                                     Optional ByVal GenerateLackingData As Boolean = False) As String

        'Counting phones to detemine if a phonetic transcription exists. This value is used to skip fields that are based on the existance of a phonetic transcription, such as sonographs
        If CurrentPhoneCount.HasValue = False Then
...
Public Class TextOnlyWord
    ...
    Public Shared Function GetPropertyNames() As List(Of String)
    ...
    Public Shared Function AfcListColumnNamesProperties() As System.Reflection.PropertyInfo()

        'Getting all properties of a TextOnlyWord
        Dim AfcListProps As New List(Of System.Reflection.PropertyInfo)
        Dim MyProperyInfo() As System.Reflection.PropertyInfo = GetType(TextOnlyWord).GetProperties
        For Each pi In MyProperyInfo
            Select Case pi.Name
                Case "PLDx_Average", "OLDx_Average", "OrthographicIsolationPoint", "PhoneticIsolationPoint", "MostCommonPoS", "MostCommonLemma", "CorrectedTranscription",
                     "ManuallyReveiwedCount", "TemporarySyllabification", "PLD1Transcriptions", "OLD1Spellings", "PLDx_Neighbors", "OLDx_Neighbors", "AllPossibleSenses",
                     "SSPP", "PSP", "PSBP", "S_PSP", "S_PSBP", "GIL2P_OT", "PIP2G_OT", "G2P_OT", "PhoneCountZero", "ManualEvaluations", "ManualEvaluationsCount", "IPA"
                Case Else
                    AfcListProps.Add(pi)
            End Select
